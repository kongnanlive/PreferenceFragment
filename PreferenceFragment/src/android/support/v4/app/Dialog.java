 ...
 * <p>
 * Note: Activities provide a facility to manage the creation, saving and restoring of dialogs. See
 * {@link Activity#onCreateDialog(int)}, {@link Activity#onPrepareDialog(int, Dialog)},
 * {@link Activity#showDialog(int)}, and {@link Activity#dismissDialog(int)}. If these methods are
 * used, {@link #getOwnerActivity()} will return the Activity that managed this dialog.
 * <p>
 * Often you will want to have a Dialog display on top of the current input method, because there is
 * no reason for it to accept text. You can do this by setting the
 * {@link WindowManager.LayoutParams#FLAG_ALT_FOCUSABLE_IM
 * WindowManager.LayoutParams.FLAG_ALT_FOCUSABLE_IM} window flag (assuming your Dialog takes input
 * focus, as it the default) with the following code:
 * 
 * <pre>
...
public class Dialog implements DialogInterface, Window.Callback, KeyEvent.Callback,
     ...
     *            window. See <a href="{@docRoot}
     *            guide/topics/resources/available-resources.html#stylesandthemes">Style
    ...
    Dialog(Context context, int theme, boolean createContextThemeWrapper) {
        ...
        try {
            ...
            w = (Window) makenewwindow.invoke(null, mContext);
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    /**
     * Returns the Activity that owns this Dialog. For example, if {@link Activity#showDialog(int)}
     ...
     * Key shortcuts can also be implemented by setting the {@link MenuItem#setShortcut(char, char)
    ...
    public boolean onTouchEvent(MotionEvent event) {
        if (mCancelable && mShowing && event.getAction() == MotionEvent.ACTION_DOWN
                && isOutOfBounds(event)) {
            cancel();
            return true;
        }
     ...
     * <p>
     * Generic motion events describe joystick movements, mouse hovers, track pad touches, scroll
     * wheel movements and other input events. The {@link MotionEvent#getSource() source} of the
     * motion event specifies the class of input that was received. Implementations of this method
     * must examine the bits in the source before processing the event. The following code example
     * shows how this is done.
     * </p>
     * <p>
     * Generic motion events with source class {@link android.view.InputDevice#SOURCE_CLASS_POINTER}
     * are delivered to the view under the pointer. All other generic motion events are delivered to
     * the focused view.
     * </p>
     * <p>
     * See {@link View#onGenericMotionEvent(MotionEvent)} for an example of how to handle this
     * event.
     * </p>
     * 
     * @param event
        ...
        while (activity == null && context != null) {
            if (context instanceof Activity) {
                ...
                context = (context instanceof ContextWrapper) ? ((ContextWrapper) context)
                        .getBaseContext() : // unwrap one level
                        null; // done
            }
        }
     ...
     * <p>
     * This will only be invoked when the dialog is canceled. Cancel events alone will not capture
     * all ways that the dialog might be dismissed. If the creator needs to know when a dialog is
     * dismissed in general, use {@link #setOnDismissListener}.
     * </p>
     * 
     * @param listener
