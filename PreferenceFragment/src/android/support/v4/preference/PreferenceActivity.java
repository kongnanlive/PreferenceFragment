...
import android.support.v4.preference.util.XmlUtils;
 ...
 * to the user. Prior to {@link android.os.Build.VERSION_CODES#HONEYCOMB} this class only allowed
 * the display of a single set of preference; this
 * functionality should now be found in the new {@link PreferenceFragment} class. If you are using
 * PreferenceActivity in its old mode, the documentation
 ...
 * {@link PreferenceFragment} to display the preferences of that header. The actual layout and
 * display of these associations can however vary; currently there are two major approaches it may
 * take:
 * <ul>
 * <li>On a small screen it may display only the headers as a single list when first launched.
 * Selecting one of the header items will re-launch the activity with it only showing the
 * PreferenceFragment of that header.
 ...
 * </ul>
 * <p>
 * Subclasses of PreferenceActivity should implement {@link #onBuildHeaders} to populate the header
 * list with the desired items. Doing this implicitly switches the class into its new "headers +
 * fragments" mode rather than the old style of just showing a single preferences list. <div
 * class="special reference">
 * <h3>Developer Guides</h3>
 * <p>
 * For information about using {@code PreferenceActivity},
 * read the <a href="{@docRoot}, read the <a href="{@docRoot}
 * guide/topics/ui/settings.html">Settings</a> guide.
 * </p>
 * </div>
 * <a name="SampleCode"></a> <h3>Sample Code</h3>
 * <p>
 * The following sample code shows a simple preference activity that has two different sets of
 * preferences. The implementation, consisting of the activity itself as well as its two preference
 * fragments is:
 * </p>
 * {@sample
 * development/samples/ApiDemos/src/com/example/android/apis/preference/PreferenceWithHeaders.java
 * activity}
 * <p>
 * The preference_headers resource describes the headers to be displayed and the fragments
 * associated with them. It is: {@sample development/samples/ApiDemos/res/xml/preference_headers.xml
 * headers}
 * <p>
 * The first header is shown by Prefs1Fragment, which populates itself from the following XML
 * resource:
 * </p>
 * {@sample development/samples/ApiDemos/res/xml/fragmented_preferences.xml preferences}
 * <p>
 * Note that this XML resource contains a preference screen holding another fragment, the
 * Prefs1FragmentInner implemented here. This allows the user to traverse down a hierarchy of
 * preferences; pressing back will pop each fragment off the stack to return to the previous
 * preferences.
 * <p>
...
public abstract class PreferenceActivity extends ListActivity implements
     ...
     * <p/>
     * Starting from Key Lime Pie, when this argument is passed in, the PreferenceActivity will call
     * isValidFragment() to confirm that the fragment class name is valid for this activity.
     */
    ...
    public static final class Header implements Parcelable {
         ...
         * it is updated. The default value is {@link PreferenceActivity#HEADER_ID_UNDEFINED},
         * meaning no id.
         * 
         ...
         * this resource is loaded from <var>res</var> and returned. Otherwise {@link #title} is
         * returned.
         */
        ...
        /**
         * Return the currently set bread crumb short title. If {@link #breadCrumbShortTitleRes} is
         * set,
         * this resource is loaded from <var>res</var> and returned. Otherwise
    ...
    /**
     * Called after the header list has been updated ({@link #onBuildHeaders} has been called and
     * returned due to {@link #invalidateHeaders()}) to
        ...
        try {
            ...
            while ((type = parser.next()) != XmlPullParser.END_DOCUMENT
                ...
                if ("header".equals(nodeName)) {
                    ...
                    header.id = sa.getResourceId(R.styleable.PreferenceHeader_id,
                            (int) HEADER_ID_UNDEFINED);
     ...
     * {@link android.os.Build.VERSION_CODES#KITKAT}. For later versions, it will throw an
     * exception.
     * 
     * @param fragmentName
    ...
    public void onHeaderClick(Header header, int position) {
        if (header.fragment != null) {
            if (mSinglePane) {
                ...
                startWithFragment(header.fragment, header.fragmentArguments, null, 0, titleRes,
                        shortTitleRes);
            } else {
                switchToHeader(header);
            }
    ...
    /**
     * Like {@link #startWithFragment(String, Bundle, Fragment, int, int, int)} but uses a 0
     * titleRes.
     */
     ...
     * This will normally be called for you. See {@link android.app.FragmentBreadCrumbs} for more
     * information.
     */
